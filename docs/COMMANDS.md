...
top of the main window) and start with `.lpc`. Callsigns are
case-insensitive. Output appears in a chat tab named **LPC**.
...
```
.lpc set DLH4TX calt FL120
.lpc set DLH4TX hdg 250
.lpc set DLH4TX mach 0.78
.lpc set BAW23K calt ils
.lpc set BAW23K sqk 2354
```
...
- The send happens in the background: the text is injected on the next
  ...
  it) and verified on the tick after that. Expect the verdict in the LPC
...
behalf. Same injection mechanism and caveats as `.lpc freq` (minus the
FREQ-key remapping issue — this one uses plain ENTER).

```
.lpc msg DLH4TX please check your assigned squawk
.lpc msg EDDF_TWR handoff coming in 2 minutes
```
...
### `.lpc pad <callsign> <text|clear>`

Sets (or clears) the scratch pad string persistently. Multi-word text is
allowed.

```
.lpc pad DLH4TX RWY CHANGE
.lpc pad DLH4TX clear
```
...
```
.lpc state DLH4TX STUP
.lpc state DLH4TX PUSH
.lpc state DLH4TX TAXI
.lpc state DLH4TX CLEA
```
...
*Reading* SID/STAR: `.lpc show <callsign>` (fields `SID:` / `STAR:`).

### `.lpc sid <callsign> <SID[/RWY]>`
### `.lpc star <callsign> <STAR>`

```
.lpc sid DLH4TX CINDY1S
.lpc sid DLH4TX CINDY1S/25R     (also assigns the departure runway)
.lpc star BAW23K ROKIL3A
```
...
- If a flight has an unusual route where the first/last token is *not* the
  procedure, review the route with `.lpc show` first — the rewrite is
...
### `.lpc gateway config <base64>` — the setup command
...
### `.lpc gateway url <url>` / `.lpc gateway token <bearer-token>`
...
### `.lpc gateway connect` / `.lpc gateway disconnect`
...
```
.lpc gateway config aHR0cHM6Ly9hcGkuZXhhbXBsZS5jb20vZXVyb3Njb3BlOjN8eDdKZC4uLg
.lpc gateway connect
.lpc gateway status
```
